...
public class Grupo implements Comparable <Grupo> {
	...
	public void eliminarAlumnoDeGrupo(String dniAlumnoAEliminar)  throws ColegioException{
		...
		if ( !alumnos.containsKey(dniAlumnoAEliminar)) {
			throw new ColegioException("No existe el alumno con el dni " + dniAlumnoAEliminar );
		}
	...
	public String toString() {
		...
		ArrayList<Alumno> listParaOrdenar=new ArrayList<Alumno>(alumnos.values());
		
		Collections.sort(listParaOrdenar);
		
		sb.append("Grupo " + nivel + "Letra " +letra + "\n");
		
		for (Alumno a: listParaOrdenar) {
